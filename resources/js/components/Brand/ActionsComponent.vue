<template>
    <div>
        <div :class="isDelete(brand.id) ? 'hidden' : 'inline-flex items-center space-x-3'">
            <a href="" @click.prevent="this.$parent.$parent.$refs.edit.changeUpdateBrandId(brand.id, brand.name, brand.image_url)"
            ...
            <a href="" @click.prevent="changeDeleteBrandId(brand.id)" title="Delete" class="hover:text-white"><svg xmlns="http://www.w3.org/2000/svg"
                    fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                    class="w-5 h-5 group-hover:text-indigo-400">
                    <path stroke-linecap="round" stroke-linejoin="round"
