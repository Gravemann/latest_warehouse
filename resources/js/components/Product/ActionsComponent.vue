<template>
    <div>
        <div :class="isDelete(product.id) ? 'hidden' : 'inline-flex items-center space-x-3'">
            <a href="#" @click.prevent="this.$parent.$parent.$refs.edit.changeUpdateProductId(product.id, product.brand_id, product.name, product.purchase_price, product.sale_price, product.quantity, product.image_url)"
            ...
            <a href="#" @click.prevent="changeDeleteProductId(product.id)" title="Delete" class="hover:text-white"><svg xmlns="http://www.w3.org/2000/svg"
                    fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                    class="w-5 h-5 group-hover:text-indigo-400">
                    <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <div :class="isDelete(product.id) ? 'inline-flex items-center space-x-3' : 'hidden'">
            <a href="#" @click.prevent="deleteProduct(product.id)" title="Confirm" class="hover:text-white"><svg 
